...
export class ClientesComponent {
  ...
  guardar() {
    this.clientesServices.postCliente(this.cliente).subscribe((data: any) => {
      this.closeModal();
      Swal.fire(
        'Registro guardado',
        '',
  ...
  editModal(registro: any){
    this.EDICION = true;
    this.cliente = registro;
    this.openModal();
  }

  editar() {
    this.clientesServices.putCliente(this.cliente).subscribe((data: any) => {
      this.registros.forEach((element: any) => {
        if(element.id == this.cliente.id){
          element.correo = this.cliente.correo;
          element.apellido = this.cliente.apellido;
          element.nombre = this.cliente.nombre;
          element.cedula = this.cliente.cedula;
        }
      });
    });
    this.closeModal();
  ...
  eliminar(registro: any){
    this.clientesServices.deleteCliente(this.cliente.id).subscribe((data: any) => {
  ...
  generarPDF(registro: any){
    ...
    this.examenesServices.getExamenes().subscribe((data: any) => {
      data.forEach((element: any) => {
        if(element.cliente == registro.cedula){
          const documentDefinition = {
            content: [
              ...
              'Cliente: ' + registro.nombre + ' ' + registro.apellido,
          ...
          pdfMake.createPdf(documentDefinition).download(registro.nombre + '.pdf');
        }
      });
    });
